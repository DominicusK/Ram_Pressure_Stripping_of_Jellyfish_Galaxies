...
npart = 199                  # Total number of particles in galaxy (gas).
...
rho0_cl=1.069597918E-13       # rho0 (kg/km^3) if dont have then use n0 and u
...
#        self.zcontrib_DMtest=self.Rcontrib_DMtest*(self.z / self.radius)
        ...
        # Components
        self.rcontrib_DM = self.contrib_DM * m.pi * GRAV * rho0DM * r0DM ** 2 * self.r / self.radius
        self.zcontrib_DM = self.contrib_DM * m.pi * GRAV * rho0DM * r0DM ** 2 * self.z / self.radius
        self.xcontrib_DM = self.contrib_DM * m.pi * GRAV * rho0DM * r0DM ** 2 * self.x / self.radius
        self.ycontrib_DM = self.contrib_DM * m.pi * GRAV * rho0DM * r0DM ** 2 * self.y / self.radius

        self.contrib_DMff= -2. * (-(r0DM / (self.height ** 2)) * m.atan(self.height / r0DM) + (1. + (r0DM / self.height)) * (r0DM / (r0DM ** 2 + self.height ** 2))) + 2. * (((-r0DM / (self.height ** 2)) * m.log(1. + (self.height / r0DM))) + (1 + (r0DM / self.height)) / (r0DM + self.height)) - (((r0DM / self.height ** 2) * m.log(1. + ((self.height ** 2) / r0DM ** 2))) + (1. - (r0DM / self.height)) * 2. * self.height / (r0DM ** 2 + self.height ** 2))

        self.zontrib_DMff = self.contrib_DM * m.pi * GRAV * rho0DM * r0DM ** 2 * self.ffglimit / self.height
...
#       self. Calc_Radius()
    ...
    def Total_Acc(self):                                             # The acceleration components in x,y,z,r from all potentials.
        ...
        self.ztot_accffg=self.zontrib_DMff+self.zcontrib_Bffg+self.zcontrib_SDffg

        self.f_gg_height=abs(self.ztot_accffg*self.surf_rho)                   # The force from the gravitational potentials as stated by Gott & Gun (2D).
        self.f_gg = abs(self.ztot_acc* self.surf_rho)
        return self.rtot_acc, self.xtot_acc, self.ytot_acc, self.zcontrib_DM,self.zcontrib_B,self.zcontrib_SD
    ...
    def Initial_Velocity(self):                                      # Initial Velocity Setup (km/s).
        ...
        self.xvel =0#-1.0 * (self.Vcirc_tot) * m.sin(self.theta)
        self.yvel =0#(self.Vcirc_tot) * m.cos(self.theta)
        self.zvel= 0
    ...
    def Cvel(self, t, dt, ram):                                      # All functions combined to evolve galaxy overtime and apply ram pressure.
        ...
        self.history[0]=[self.x, self.y, self.r, self.radius, self.Vcirc_DM, self.Vcirc_B, self.Vcirc_SD, self.z, self.rplot,self.zvel, self.f_gg,self.rcontrib_DM,self.ztot_acc,self.Vcirc_tot] # list contains parameters wanted from class (before time evolution).
...
#               F = ((ram[1] - ram[0]) / (dt))
                ...
                self.Ram_Pressure(I-ram[0],h_rp,c_rpv,v_wind[I_develop],rho_wind,r_cluster[I_develop],dt_rp,wind=wind_type)   # ICM density (Kg/Km^3) (conversion from g/cm^3 is 1E12).
 #               if int(F) > 0:
 #                   ramhistory_shape = (int(F), 1)
 #                   self.ramhistory = np.ndarray(ramhistory_shape)
 #                   self.ramhistory[int(F-1)] = [self.pram_max]
            J=int(I/dt)
            self.history[J]=[self.x, self.y, self.r, self.radius, self.Vcirc_DM, self.Vcirc_B, self.Vcirc_SD, self.z, self.rplot,self.zvel,self.f_gg,self.rcontrib_DM,self.ztot_acc,self.Vcirc_tot] # list contains parameters wanted from class (after time evolution).
...
#                if self.pramzvelrel > self.pram_tv:
            ...
            # Constant RP Equation.
                self.pram_orig=rho_w*(v_w-self.zvel)**2
            ...
            self.ramhistory.append(self.pram_tvrel)


# Functions to manipulate class data.
def mass_disk(RAMs, time, limit):
    massleft = 0
...
def mass_tails(RAMs,time,boundary, limit):
    masstails=0
    for particle in RAMs:
        z=particle.history[time][7]
        if boundary < z < limit:
...
def velocity_distribution(RAMs,time,boundary):
    z_position=[]
    z_velocity=[]
    for particle in RAMs:
        z=particle.history[time][7]
...
def ICM_Density_CRP(rp,v,cw):
    rho_icm=[]
    rho_icm.append(rp/((v**2)*0.5*cw))
    return rho_icm